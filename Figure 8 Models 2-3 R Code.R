...
QM2 <-
  nlsLM(
    ...
    control = nls.control(
      ...
      printEval = FALSE,
      warnOnly = TRUE))
#Coef(QM2) gives the estimated parameters A and C.
coef(QM2)

#Verifying that intial conditions on the parameters satisfy the assumptions in the Theorem
exp(2)
1+0.1^2/(1-2*0.1)

#Verifying that the estimated parameters satisfy the assumptions in the Theorem
exp(coef(QM2)[1])
...
#estimated parameters are outside theorem's assumptions
exp(1/(coef(QM3)[1]*coef(QM3)[2]))
1+(1/coef(QM3)[3]^4)/(1-2/coef(QM3)[2]^2)
