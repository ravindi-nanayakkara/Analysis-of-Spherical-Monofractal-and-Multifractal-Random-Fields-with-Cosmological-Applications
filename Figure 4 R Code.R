...
####################### Large Window ####################################
#Here, a realization of a multifractal random field is simulated in a large spherical window using a Gaussian mother random
#field $Y(x)$ with the exponential covariance model
...
######### Figure 4(a) gives the plot of the simulated multifractal random field
...
#This figure plots the Sample Renyi function with the linear function for the simulated large window of sky data.
plot(Tq[,1], Tq[,2], ylab ="T(q)", xlab = "q", pch = 20, col = "blue", cex.main=1.25, cex.lab=1.25, cex.axis=1)
segments(Tq[1,1], Tq[1,2], Tq[N,1], Tq[N,2], lwd = (2), col= "red")

Tq[,3] <- (Tq[,1]*(Tq[1,2]-Tq[N,2])+Tq[N,2]*Tq[1,1]-Tq[1,2]*Tq[N,1])/(Tq[1,1]-Tq[N,1])

#This figure plots the difference between the Sample Renyi function and the linear function for the simulated large window of sky data.
plot(Tq[,1],Tq[,2]-Tq[,3], ylab ="difference", xlab = "q", pch = 20, col = "blue")
...
######### Figure 4(b)-This figure gives the plot of the Sample Renyi function with the fitted log-normal model for 
#the simulated large window of sky data.

plot(Tq[,1],Tq[,2], ylab ="T(q)", xlab = "q", pch = 20, col = "blue")
lines(Tq[,1],Q11, col="red", lwd=2)

#This figure gives the plot of the difference between the Sample Renyi function and the fitted log-normal model for
#the simulated large window of sky data.
plot(Tq[,1],Q11-Tq[,2], ylab ="difference", xlab = "q", pch = 20, col = "blue")
...
#Computes the alpha function for the simulated large window of sky data.
...
########### Figure 4c-This figure gives the plot of the function f(alpha) versus alpha for the simulated large window of sky data.
